...
bot.onText(/\/start/, (msg) => {
  ...
  const intros = [
    `${greet}\n\n💛 Hey Hawikoo! Ani Tamebot dha — Temesgen sif jedhe anan nan hojjate.\nYeroo inni hin jirre, ani si waliin jira. Hawikoo Maal si gargaaruu danda'a? 😊`,
    `${greet}\n\n✨ Hawiko Akkam nagaa keeti! Ani Tamebot, gargaaraa kee. Waan si barbaachisu hundumaa na gaafachuu dandeessa. Si gargaaruun gammachuudha! 🌸`,
    `${greet}\n\n🌸 Hawiko Nagaa Hawiko akkami jirta ! Temesgen sif  akka yaaduf  na ajaje.\nAkkam naga jirta oolman kee hoo? 💛`,
    `${greet}\n\n😊 Hawkio Ani Tamebot, michuu kee fi nama si cina jirudha.\nWaan barbaaddu na gaafadhu — as jira! 🌸`,
  ];
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (err.message?.includes('429') || err.message?.includes('RESOURCE_EXHAUSTED')) {
      await bot.sendMessage(chatId,
        '🌸 sorry Hawi… daqiiqaa muraasa booda yaali.\n! 💛'
      );
      return;
    }
    await bot.sendMessage(chatId, playfulFallback());
